...
def init_c(DataLoader, net, eps=0.1):
    ...
    with torch.no_grad():
        for index, (images, label) in enumerate(tqdm(DataLoader, position=0)):
            ...
            img_tmp = img_org.unfold(2, 32, p_stride).unfold(3, 32, p_stride)     # p_stride = 16
...
def train(args, NORMAL_NUM, generator, discriminator, optimizer_g, optimizer_d):
    ...
    test_auc = 0
    BEST_AUC = 0
    train_path = '/home/yszhao/Projects/PublicDataset/mvtec_annomaly_detection/{}/train/good'.format(NORMAL_NUM)
    START_ITER = 0
    ...
    generator.c = init_c(train_dataset_loader, generator)    #  Center of Gaussian Anomaly Classifier  --zys
    generator.sigma = init_sigma(train_dataset_loader, generator)        #  sigma means alhpa in IGD  --zys
    ...
    for iteration in process:
        poly_lr_scheduler(optimizer_d, init_lr=LR, iter=iteration, max_iter=END_ITER)      #   adjust learning rate  --zys
        ...
        batch_data = batch.to(device)    # [2, 3, 256, 256]   --zys
        '''Devide the image[256, 256] to many blocks, in this code, to[32, 32]    --zys'''
        data_tmp = batch_data.unfold(2, 32, p_stride).unfold(3, 32, p_stride)    #  [2, 3, 15, 15, 32, 32]  --zys
        data_tmp = data_tmp.permute(0, 2, 3, 1, 4, 5)   # [2, 15, 15, 3, 32, 32]  --zys

        real_data = data_tmp.reshape(-1, 3, 32, 32)    #  [450, 3, 32, 32]  --zys

        # --------------------- TRAIN E ------------------------
        optimizer_g.zero_grad()
        latent_z = generator.encoder(real_data)
        fake_data = generator(real_data)     #  encoder and decoder, fake_data is x_hat  --zys

        b,_ = latent_z.shape
        # Reconstruction loss
        weight = 0.85
        # print(real_data.shape, fake_data.shape)    # --zys
...
def validation(NORMAL_NUM, iteration, generator, discriminator, real_data, fake_data, is_end, AUC_LIST, END_ITER):
    ...
    normal_recon = []
    abnormal_recon = []
    test_root = '/home/yszhao/Projects/PublicDataset/mvtec_annomaly_detection/{}/test/'.format(NORMAL_NUM)
...
if __name__ == "__main__":
    ...
    for key in sorted(sub_category):    # 1~15 categories  --zys
        NORMAL_NUM = sub_category[key]
        # NORMAL_NUM = category[key] if key.isdigit() else key
        print('Current Item: {}'.format(NORMAL_NUM))

        train_path = '/home/yszhao/Projects/PublicDataset/mvtec_annomaly_detection/{}/train/'.format(NORMAL_NUM)
        test_root = '/home/yszhao/Projects/PublicDataset/mvtec_annomaly_detection/{}/test/'.format(NORMAL_NUM)
        gt_root = '/home/yszhao/Projects/PublicDataset/mvtec_annomaly_detection/{}/ground_truth/'.format(NORMAL_NUM)

        # current_ckpt = "23999"
        Experiment_name = 'No.{}_p32_IGD'.format(str(NORMAL_NUM), sig_f, rec_f, svd_f)
        # Experiment_name = 'No.{}_vae'.format(str(NORMAL_NUM))

        recorder = Recorder(Experiment_name, 'MVTec_No.{}'.format(str(NORMAL_NUM)))     #  tensorboard recoder  --zys
        ...
        generator     = twoin1Generator(64, latent_dimension=latent_dimension)      # encoder and decoder --zys
        discriminator = VisualDiscriminator(64)         #  critic module  --zys

        path = './Encoder_lr_scheduler_ckpt'
        # generator.pretrain.load_state_dict(torch.load(path))    #   --zys
