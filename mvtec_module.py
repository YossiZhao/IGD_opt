...
class DepthToSpace(torch.nn.Module):
    def __init__(self, block_size):   #  block_size = 2
    ...
    def forward(self, x):
        ...
        output = output.contiguous()    # make output contiguous in memory.  --zys
...
class UpSampleConv(torch.nn.Module):
    ...
    def forward(self, x):
        output = x     #  (-1, 512, 4, 4)  --zys
        output = torch.cat((output, output, output, output), 1)    #  (-1, 512, 4, 4) --> (-1, 2048, 4, 4)  --zys
        output = self.depth_to_space(output)   #  (-1,4, 4, 2048) --> (-1, 512, 8, 8)  --zys
        output = self.conv(output)
        return output
...
class twoin1Generator256(torch.nn.Module):
    ...
    def generate(self, x):
        output = self.ln1(x)   #  128 --> 8192  --zys
        output = output.view(-1, 8 * self.dim, 4, 4)    # size:-1, 512, 4, 4   --zys
