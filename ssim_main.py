...
def train(args, NORMAL_NUM,
    ...
    for iteration in process:
        ...
        ############ Interplote ############
        e1 = torch.flip(latent_z, dims=[0])  # reverse x, eg: [1, 2, 3, 4]--> [4, 3, 2, 1]  --zys
        alpha = torch.FloatTensor(b, 1).uniform_(0, 0.5).to(device)      #   We need this in our code   --zys
        e2 = alpha * latent_z + (1 - alpha) * e1   #  z2 in paper,
        g2 = generator.generate(e2)    #  generate z2 based on x1-->z1 and z2   --zys
        reg_inter = torch.mean(discriminator(g2) ** 2)      # critic process, to predict the interpolation coefficient  --zys

        ############ GAC ############    Gaussian Anomaly Classifier  --zys
        diff = (latent_z - generator.c) ** 2
        dist = -1 * (torch.sum(diff, dim=1) / generator.sigma)
        svdd_loss = torch.mean(1 - torch.exp(dist))

        encoder_loss = ms_ssim_l1 + svdd_loss + 0.1 * reg_inter   #  0.1 is lagrange multipliers.  --zys
        encoder_loss.backward()
        optimizer_g.step()

        ############ Discriminator ############
        optimizer_d.zero_grad()
        g2 = generator.generate(e2).detach()    #  generate g2(z2 hat) again and make it detach  --zys
        fake_data = generator(real_data).detach()    #  generate fake_data again and make it detach  --zys
        ...
        d_loss = d_loss_front + d_loss_back     #  loss of adversarial loss in (10, paper)
        d_loss.backward()
        optimizer_d.step()
...
if __name__ == "__main__":
    ...
    save_root_path = os.path.join(save_root_path, "IGD_wo_inter")     #  ./p256/check_points/IGD_wo_inter  --zys
    create_dir(save_root_path)
    ckpt_path = os.path.join(save_root_path, "p256_SR-{}".format(args.sample_rate))    #  ./p256/check_points/IGD_wo_inter/p256_SR-1.0   --zys
    create_dir(ckpt_path)
    ckpt_path = os.path.join(ckpt_path, Experiment_name)      #  ./p256/check_points/IGD_wo_inter/p256_SR-1.0/No.1_p256_IGD
